...
انتباه! عندما تريد تكبير الخط دون أن يكون النص عنوانًا فعليًا لا تستخدم الـ tags المخصصة للعناوين، بل استخدم CSS عن طريق الخاصية `font-size`.
...
يعتبر الوسم `<form>` من العناصر المهمة في HTML وذلك لأنه يضم بداخله عدة عناصر عن طريقها يستطيع المستخدم إرسال البيانات إلى خادم الويب (web server) المحدد في السمة `action` الخاصة بالنموذج، ببساطة يمكنك اعتبار وظيفة الوسم `<form>` تكمن في تحديد البيانات المراد إدخالها من المستخدم.
...
مربعات الاختيار checkbox مستقلة عن بعضها بطبيعتها، فيستطيع المستخدم اختيار عدة خيارات معًا مهما كانت قيمة السمة `name`؛ السمة `name` تؤثر فقط على شكل البيانات المرسلة عند تسليم النموذج
